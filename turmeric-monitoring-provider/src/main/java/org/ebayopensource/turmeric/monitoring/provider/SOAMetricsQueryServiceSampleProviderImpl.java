...
public class SOAMetricsQueryServiceSampleProviderImpl implements
        SOAMetricsQueryServiceProvider
{
	...
	private static void loadRequestData(){
		...
		populateList(methodPath, getMetricsMetadataReqList, GetMetricsMetadataRequest.class, Integer.parseInt(prop.getProperty("getMetricsMetadata")));
		methodPath = basePath + "getMetricValue/";
		populateList(methodPath, getMetricValueReqList, GetMetricValueRequest.class, Integer.parseInt(prop.getProperty("getMetricValue")));
		methodPath = basePath + "getMetricsData/";
		populateList(methodPath, getMetricsReqList, GetMetricsRequest.class, Integer.parseInt(prop.getProperty("getMetricsData")));
	...
	private static <T> void populateList(String methodPath, List<T> list, Class<T> ref, int numFiles){
		...
		for (int i=0; i<numFiles; i++){
			...
			request = JAXB.unmarshal(cl.getResourceAsStream(methodPath + "request/" + "request" + (i+1) + ".xml"), ref);
			list.add(request);
		}
	}
	...
	private static int findRequestFileIndex(String resourceEntityType,
		...
		for (int i=0; i<getMetricsMetadataReqList.size(); i++){
			...
			entityResponseType = (request.getResourceEntityResponseType() != null)?request.getResourceEntityResponseType().value():null;
			entityNames.addAll(request.getResourceEntityName());
			if (!resourceEntityType.equalsIgnoreCase(entityType) || !stringEquals(resourceEntityResponseType, entityResponseType)){
				continue;
			}
	...
	private static int findRequestFileIndex(CriteriaInfo criteriaInfo, long startTime, long duration, int aggregationPeriod,
			String autoDelay){
		int result = 0;
		...
		for (int i=0; i<getMetricValueReqList.size(); i++){
			...
			/*if (!(criteriaInfoRef.getServiceConsumerType().equals(criteriaInfo.getServiceConsumerType()) &&
					...
					criteriaInfoRef.getRoleType().equals(criteriaInfo.getRoleType()))){
				continue;
			}*/
			if (!(stringEquals(criteriaInfoRef.getServiceConsumerType(),criteriaInfo.getServiceConsumerType()) &&
					...
					stringEquals(criteriaInfoRef.getMachineName(), criteriaInfo.getMachineName()) &&
					stringEquals(criteriaInfoRef.getRoleType(), criteriaInfo.getRoleType()))){
				continue;
			}
			if (startTimeRef == startTime &&
					durationRef == duration &&
					aggregationPeriodRef == aggregationPeriod &&
	...
	private static int findRequestFileIndex(MetricCriteria metricCriteria, MetricResourceCriteria metricResourceCriteria){
		...
		for (int i=0; i<getMetricsReqList.size(); i++){
			...
			if (!(metricCriteriaRef.getFirstStartTime() == metricCriteria.getFirstStartTime() &&
					...
					stringEquals(metricCriteriaRef.getRoleType(), metricCriteria.getRoleType()) &&
					stringEquals(metricCriteriaRef.getAutoDelay(), metricCriteria.getAutoDelay()))){
				continue;
			}
			
			if (!stringEquals(metricResourceCriteria.getResourceEntityResponseType(), metricResourceCriteriaRef.getResourceEntityResponseType())){
				continue;
			}
	...
	public List<PolicyMetricGraphData> getPolicyMetricDetailData(String policyName,
			String serviceName, String operationName, String subjectTypeName,
			String subjectValue, String listType, long startTime, long endTime) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public List<String> getMetricsMetadata(String resourceEntityType,
		...
		if (responseFileIndex > 0){
			response = getResponseData(responseFileIndex, basePath + "getMetricsMetadata/", GetMetricsMetadataResponse.class);
		}
		return response.getResourceEntityResponseNames();
	}

	/*MetricData metricData1, metricData2;
	MetricGraphData metricGraphData1, metricGraphData2;
	ErrorViewData errorViewData1, errorViewData2;
	ErrorInfos errorInfos;
	MetricGroupData metricGroupData1, metricGroupData2;
	PolicyMetricData policyMetricData1, policyMetricData2;
	*/
	/*List<CriteriaInfo> criteriaInfoList = new ArrayList<CriteriaInfo>();
	List<PolicyMetricData> policyMetricDataList = new ArrayList<PolicyMetricData>();
	List<MetricGroupData> metricGroupDataList = new ArrayList<MetricGroupData>();
	List<MetricGraphData> metricGraphDataList = new ArrayList<MetricGraphData>();
	List<MetricData> metricDataList = new ArrayList<MetricData>();
	List<ErrorViewData> errorViewDataList = new ArrayList<ErrorViewData>();
	List<ErrorInfos> errorInfosList = new ArrayList<ErrorInfos>();
	List<String> metricsMetadataList = new ArrayList<String>();

	private static final String csvPath = "/META-INF/metrics/data/";
	private static final String criteriaInfoFilePath = csvPath+"CriteriaInfo.csv";
	private static final String metricDataFilePath = csvPath+"MetricData.csv";
	private static final String metricGraphDataFilePath = csvPath+"MetricGraphData.csv";
	private static final String metricGroupDataFilePath = csvPath+"MetricGroupData.csv";
	private static final String policyMetricDataFilePath = csvPath+"PolicyMetricData.csv";
	private static final String errorViewDataFilePath = csvPath+"ErrorViewData.csv";
	private static final String errorInfosFilePath = csvPath+"ErrorInfos.csv";
	private static final String metricsMetadataFilePath = csvPath+"MetricsMetadata.csv";
	...
	public List<ErrorViewData> getErrorMetricsData(String errorType,
			List<String> serviceName, List<String> operationName,
			List<String> consumerName, String errorId, String errorCategory,
			String errorSeverity, String isErrorId,
			MetricCriteria metricCriteria) {
		return errorViewDataList;
	}

	@Override
	public ErrorInfos getErrorMetricsMetadata(String errorId, String serviceName) {
		return errorInfosList.get(0);
	}
	...
	@Override
	public List<MetricGraphData> getMetricValue(CriteriaInfo criteriaInfo,
			long startTime, long duration, int aggregationPeriod, String autoDelay) {
		return metricGraphDataList;
	}
	...
	@Override
	public List<PolicyMetricData> getPolicyMetricDetailData(String policyName,
			String serviceName, String operationName, String subjectTypeName,
			String subjectValue, String listType, long startTime, long endTime) {
		return policyMetricDataList;
	}
	...
	private void initDummyValues(){
		...
		try{
			filePath = criteriaInfoFilePath;
			populateCriteriaInfo(filePath);

			filePath = policyMetricDataFilePath;
			populatePolicyMetricData(filePath);

			filePath = metricGroupDataFilePath;
			populateMetricGroupData(filePath);

			filePath = metricGraphDataFilePath;
			populateMetricGraphData(filePath);

			filePath = metricDataFilePath;
			populateMetricData(filePath);

			filePath = errorViewDataFilePath;
			populateErrorViewData(filePath);

			filePath = errorInfosFilePath;
			populateErrorInfosData(filePath);

			filePath = metricsMetadataFilePath;
			populateMetricsMetadata(filePath);
		}catch (Exception e) {
			System.out.println("Exception while loading data from CSV : " + filePath);
			e.printStackTrace();
		}

	}

	private void populateCriteriaInfo(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		String[] values;
		int index;
		...
		while ((str = br.readLine()) != null && str.length() > 0){
			...
			values = str.split(",");
			criteriaInfo = new CriteriaInfo();
			index = 0;
			criteriaInfo.setServiceConsumerType(values[index]);
			criteriaInfo.setMetricName(values[++index]);
			criteriaInfo.setServiceName(values[++index]);
			criteriaInfo.setOperationName(values[++index]);
			criteriaInfo.setConsumerName(values[++index]);
			criteriaInfo.setPoolName(values[++index]);
			criteriaInfo.setMachineName(values[++index]);
			criteriaInfo.setRoleType(values[++index]);
		...
		try {
			...
			throw new Exception("Exception while closing file streams",e);
		}

	}

	private void populatePolicyMetricData(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		String[] values;
		int index;
		PolicyMetricData policyMetricData;
		boolean header = true;
		while ((str = br.readLine()) != null && str.length() > 0){
			if (header){
				header = false;
			...
			values = str.split(",");
			policyMetricData = new PolicyMetricData();
			index = 0;
			policyMetricData.setHitCount(values[index]);
			policyMetricData.setListType(values[++index]);
			policyMetricData.setSubjectType(values[++index]);
			policyMetricData.setSubjectValue(values[++index]);
			policyMetricData.setEffectDuration(Long.parseLong(values[++index]));
			policyMetricData.setPolicyName(values[++index]);
			policyMetricData.setOperationName(values[++index]);
			policyMetricData.setServiceName(values[++index]);
			policyMetricData.setEffect(values[++index]);
			policyMetricDataList.add(policyMetricData);
		}
		try {
			br.close();
			is.close();
	...
	private void populateMetricGroupData(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		String[] values;
		int index;
		MetricGroupData metricGroupData;
		boolean header = true;
		while ((str = br.readLine()) != null && str.length() > 0){
			if (header){
				header = false;
			...
			values = str.split(",");
			metricGroupData = new MetricGroupData();
			index = 0;
			metricGroupData.setDiff(Double.parseDouble(values[index]));
			metricGroupData.setCount2(Double.parseDouble(values[++index]));
			metricGroupData.setCount1(Double.parseDouble(values[++index]));
			metricGroupDataList.add(metricGroupData);

		}
		CriteriaInfo criteriaInfo;
		for(int i=0; i<metricGroupDataList.size(); i++){

			criteriaInfo = criteriaInfoList.get(i);
			metricGroupDataList.get(i).setCriteriaInfo(criteriaInfo);
	...
	private void populateMetricGraphData(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		String[] values;
		...
		while ((str = br.readLine()) != null && str.length() > 0){
			...
			values = str.split(",");
			metricGraphData = new MetricGraphData();
			index = 0;
			metricGraphData.setTimeSlot(Long.parseLong(values[index]));
			metricGraphData.setCriteria(values[++index]);
			metricGraphData.setCount(Double.parseDouble(values[++index]));
	...
	private void populateMetricData(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		String[] values;
		...
		while ((str = br.readLine()) != null && str.length() > 0){
			...
			metricData.setResponseTime1(Double.parseDouble(values[++index]));
		...
		for(int i=0; i<metricDataList.size(); i++){
			...
			criteriaInfo = criteriaInfoList.get(i);
			metricDataList.get(i).setCriteriaInfo(criteriaInfo);
	...
	private void populateErrorViewData(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		String[] values;
		...
		while ((str = br.readLine()) != null && str.length() > 0){
			...
			values = str.split(",");
			errorViewData = new ErrorViewData();
			index = 0;
			errorViewData.setCriteria(values[index]);
			errorViewData.setRatioDiff(Double.parseDouble(values[++index]));
			...
			errorViewData.setErrorCount1(Long.parseLong(values[++index]));
			errorViewDataList.add(errorViewData);
		}
	...
	private void populateErrorInfosData(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		String[] values;
		...
		while ((str = br.readLine()) != null && str.length() > 0){
			...
			index = 0;
			errorInfos.setId(values[index]);
			errorInfos.setName(values[++index]);
			errorInfos.setCategory(values[++index]);
			errorInfos.setSeverity(values[++index]);
			errorInfos.setDomain(values[++index]);
			errorInfos.setSubDomain(values[++index]);
			errorInfosList.add(errorInfos);
		}
	...
	private void populateMetricsMetadata(String fileName)throws Exception{
		InputStream is = SOAMetricsQueryServiceSampleProviderImpl.class.getResourceAsStream(fileName);
		BufferedReader br = new BufferedReader(new InputStreamReader(is));
		String str;
		while ((str = br.readLine()) != null && str.length() > 0){

			metricsMetadataList.add(str);
		}
	...
	/*public static void main(String args[]){
		SOAMetricsQueryServiceSampleProviderImpl obj = new SOAMetricsQueryServiceSampleProviderImpl();
		List<String> names = new ArrayList<String>();
		names.add("BulkDataExchangeService");
		names.add("MetadataDependencyService");
		names.add("PolicyService");
		names.add("ShippingCalculatorService");
		List<String> list = obj.getMetricsMetadata("Service", names, "Operation");
		for(String str :list){
			System.out.println(str);
		}

		GetMetricsMetadataRequest request = new GetMetricsMetadataRequest();
		request = JAXB.unmarshal(new File("meta-src/META-INF/metrics/data/getMetricsMetadata/request/request3.xml"), GetMetricsMetadataRequest.class);
		List<String> result = obj.getMetricsMetadata(request.getResourceEntityType().value(), request.getResourceEntityName(), request.getResourceEntityResponseType().value());
		...
		GetMetricValueRequest request = new GetMetricValueRequest();
		request = JAXB.unmarshal(new File("meta-src/META-INF/metrics/data/getMetricValue/request/request6.xml"), GetMetricValueRequest.class);
		List<MetricGraphData> result = obj.getMetricValue(request.getCriteriaInfo(), request.getStartTime(), request.getDuration(), request.getAggregationPeriod(), request.getAutoDelay());
		for(MetricGraphData data : result){
			System.out.println(data.getTimeSlot()+","+data.getCount());

		}

		GetMetricsRequest request = new GetMetricsRequest();
		request = JAXB.unmarshal(new File("meta-src/META-INF/metrics/data/getMetricsData/request/request82.xml"), GetMetricsRequest.class);
		List<MetricGroupData> result = obj.getMetricsData(request.getMetricCriteria(), request.getMetricResourceCriteria());
		for(MetricGroupData data : result){
			System.out.println(data.getCriteriaInfo().getConsumerName()+","+data.getDiff()+","+data.getCount1()+","+data.getCount2()+","+data.getCriteriaInfo().getMetricName());
		}
