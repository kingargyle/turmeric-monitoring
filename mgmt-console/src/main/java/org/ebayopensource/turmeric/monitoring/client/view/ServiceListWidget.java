...
public class ServiceListWidget extends Composite {
	...
	public void setServicesMap(Map<String, Set<String>> map) {
		...
		Label rootLabel = new Label(ConsoleUtil.constants.services() + "("
				+ map.size() + ")");
		...
		for (Map.Entry<String, Set<String>> e : map.entrySet()) {
			...
			for (String s : e.getValue()) {
				...
				if (selectedOperation != null && selectedOperation.equals(s)
				...
				while (selectedItem == null && itor.hasNext()) {
					...
					if (item.getChildCount() > 0) {
						...
						if (selectedService != null
								&& selectedService.equals(item.getText())
						...
						// operation match
						if (selectedService != null
								&& selectedService.equals(item.getParentItem()
										.getText())
								&& selectedOperation != null
								&& selectedOperation.equals(item.getText())) {
							selectedItem = item;
						}
					}
				}
