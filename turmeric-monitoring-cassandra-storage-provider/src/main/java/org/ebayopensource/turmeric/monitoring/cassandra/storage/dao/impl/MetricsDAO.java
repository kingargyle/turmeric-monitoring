...
public class MetricsDAO {
   ...
   public MetricsDAO(String clusterName, String host, String keyspaceName) {
      ...
      keySpace = hectorManager.getKeyspace(clusterName, host, keyspaceName, "ServiceOperationByIp", true, String.class,
               String.class);
   }

   /**
    * Creates the key for ip date and service name.
    * 
    * @return the string
    */
    ...
    * @return the string
    */
   public String createKeyForMetricValue(String ipAddress, MetricIdentifier cmetricIdentifier, String consumerName,
            long now) {
      return ipAddress + KEY_SEPARATOR + cmetricIdentifier.getServiceAdminName() + KEY_SEPARATOR
               ...
               + cmetricIdentifier.getMetricName() + KEY_SEPARATOR + now;
   }
   ...
   /**
    * Creates the key for time series by consumer.
    * 
    * @param ipAddress
    *           the ip address
    * @param cmetricIdentifier
   ...
   public void saveMetricValues(String ipAddress, MetricIdentifier cmetricIdentifier, long now, int snapshotInterval,
      ...
      for (MetricComponentValue metricComponentValue : metricComponentValues) {
         ...
         List<HColumn<String, String>> metricTimeSeriesColumn = Arrays.asList(HFactory.createColumn(metricValueKey, "",
                  STR_SERIALIZER, STR_SERIALIZER));
         HSuperColumn<Long, String, String> metricTimeSeriesSuperColumn = HFactory.createSuperColumn(now,
                  metricTimeSeriesColumn, LNG_SERIALIZER, STR_SERIALIZER, STR_SERIALIZER);
         metricTimeSeriesMutator.insert(timeSeriesKey, "MetricTimeSeries", metricTimeSeriesSuperColumn);

         System.out.println("saving MetricTimeSeries CF with key=" + timeSeriesKeyByConsumer);
         System.out.println(", column key = " + now);
         System.out.println(", column value = " + metricValueKey);
         metricTimeSeriesMutator.insert(timeSeriesKeyByConsumer, "MetricTimeSeries", metricTimeSeriesSuperColumn);
      }
