...
public class MetricDAOTest extends CassandraTestHelper {
   ...
   public void init() throws TTransportException, IOException, InterruptedException, ConfigurationException {
      initialize();
      metricDAO = new MetricsDAO(cluster_name, cassandra_node_ip, keyspace_name);
   }
   ...
   public void testCreateKeyForMetricValuesByIpAndDate() {
      String actual = metricDAO.createKeyForMetricValuesByIpAndDate("someip", System.currentTimeMillis());
      String date = new SimpleDateFormat("ddMMyyyy").format(new Date(System.currentTimeMillis()));
