 ...
 */
public class DAOSOAMetricsQueryServiceProvider implements
		SOAMetricsQueryServiceProvider {
	private final SOAMetricsQueryServiceProvider delegate;

	/**
	 * Constructor used by the XML configuration, it must hard-code all
	 * dependencies.
	 */
	...
	public DAOSOAMetricsQueryServiceProvider(
		...
		ClassLoader classLoader = SOAMetricsQueryServiceProvider.class
				.getClassLoader();
		Class[] interfaces = { SOAMetricsQueryServiceProvider.class };
		JPAAroundAdvice handler = new JPAAroundAdvice(entityManagerFactory,
				new Target());
		this.delegate = (SOAMetricsQueryServiceProvider) Proxy
	...
	public List<MetricData> getDetailData(String dc,
		...
		return delegate.getDetailData(dc, metricCriteria,
	...
	public List<MetricGraphData> getErrorGraph(String serviceName,
			...
			MetricCriteria metricCriteria) {
		return delegate.getErrorGraph(serviceName, operationName, consumerName,
				errorId, errorCategory, errorSeverity, metricCriteria);
	}
	...
	public List<ErrorViewData> getErrorMetricsData(String errorType,
			...
			MetricCriteria metricCriteria) {
		return delegate.getErrorMetricsData(errorType, serviceName,
				operationName, consumerName, errorId, errorCategory,
				errorSeverity, errorName, metricCriteria);
	}
	...
	public ErrorInfos getErrorMetricsMetadata(String errorId, String errorName,
			String serviceName) {
		return delegate
				.getErrorMetricsMetadata(errorId, errorName, serviceName);
	}
	...
	public MetricData getMetricSummaryData(String dc,
			MetricCriteria metricCriteria,
			MetricResourceCriteria metricResourceCriteria) {
		return delegate.getMetricSummaryData(dc, metricCriteria,
				metricResourceCriteria);
	}
	...
	public List<MetricGraphData> getMetricValue(CriteriaInfo criteriaInfo,
			long startTime, long duration, int step, String autoDelay) {
		return delegate.getMetricValue(criteriaInfo, startTime, duration, step,
				autoDelay);
	}
	...
	public List<PolicyMetricData> getPolicyMetricData(long startTime,
		...
		return delegate.getPolicyMetricData(startTime, endTime, policyType,
				policyName, serviceName, operationName, subjectTypeName,
				subjectValue, effect);
	}
	...
	public List<PolicyMetricGraphData> getPolicyMetricDetailData(
		...
		return delegate.getPolicyMetricDetailData(policyName, serviceName,
				operationName, subjectTypeName, subjectValue, listType,
				startTime, endTime);
	}
	...
	public List<String> getMetricsMetadata(String resourceEntityType,
		...
		return delegate.getMetricsMetadata(resourceEntityType,
				resourceEntityName, resourceEntityResponseType);
	}
	...
	private static class Target implements SOAMetricsQueryServiceProvider {
		...
		@Override
		public List<MetricData> getCustomReportData(
				ReportCriteria reportCriteria, MetricCriteria metricCriteria) {
			return null;
		}
		...
		@Override
		public List<ErrorViewData> getErrorMetricsData(String errorType,
				List<String> serviceNames, List<String> operationNames,
				List<String> consumerNames, String errorIdString,
				String errorCategoryString, String errorSeverityString,
				String isErrorId, MetricCriteria metricCriteria) {
			long firstStartTime = metricCriteria.getFirstStartTime();
			long secondStartTime = metricCriteria.getSecondStartTime();
			long duration = TimeUnit.SECONDS.toMillis(metricCriteria
					.getDuration());
			...
			String errorCategory = errorCategoryString == null ? null
					: ErrorCategory.fromValue(errorCategoryString).value();
			String errorSeverity = errorSeverityString == null ? null
					: ErrorSeverity.fromValue(errorSeverityString).value();
			...
			if ("Category".equals(errorType)) {
				errors1 = errorDAO.findErrorValuesByCategory(firstStartTime,
						firstStartTime + duration, serverSide,
						aggregationPeriod, errorId, errorCategory,
						errorSeverity, filters);
				calls1 = metricsDAO.findCallsCount(firstStartTime,
						firstStartTime + duration, serverSide,
						aggregationPeriod);
				errors2 = errorDAO.findErrorValuesByCategory(secondStartTime,
				...
				errors1 = errorDAO.findErrorValuesBySeverity(firstStartTime,
						firstStartTime + duration, serverSide,
						aggregationPeriod, errorId, errorCategory,
						errorSeverity, filters);
				calls1 = metricsDAO.findCallsCount(firstStartTime,
						firstStartTime + duration, serverSide,
						aggregationPeriod);
				errors2 = errorDAO.findErrorValuesBySeverity(secondStartTime,
						secondStartTime + duration, serverSide,
						aggregationPeriod, errorId, errorCategory,
						errorSeverity, filters);
				calls2 = metricsDAO.findCallsCount(secondStartTime,
			...
			for (Map.Entry<String, Map<String, Object>> entry : map1.entrySet()) {
				...
				errorViewData.setErrorCallRatio1(calls1 == 0 ? 0 : count1
						/ calls1);
				...
				if (row2 != null) {
					...
					errorViewData.setErrorCallRatio2(calls2 == 0 ? 0 : count2
							/ calls2);

					double errorDiff = calcDiff(Double.valueOf(count1),
							Double.valueOf(count2));
					// if (count2 == 0)
					// if (count1 == 0)
					// errorDiff = 0.0D;
					// else
					...
					errorViewData
							.setErrorDiff(errorViewData.getErrorCount1() == 0 ? 0
									: -100);
					errorViewData.setRatioDiff(errorViewData
							.getErrorCallRatio1() == 0 ? 0 : -100);
				}
			...
			Collections.sort(result, new Comparator<ErrorViewData>() {
				...
				public int compare(ErrorViewData evd1, ErrorViewData evd2) {
					...
					if (v1 == v2) {
						v1 = evd1.getErrorCallRatio1()
								+ evd1.getErrorCallRatio2();
						v2 = evd2.getErrorCallRatio2()
								+ evd2.getErrorCallRatio2();
					}
		...
		@Override
		public ErrorInfos getErrorMetricsMetadata(String errorId,
				String errorName, String serviceName) {
			long id = Long.parseLong(errorId);
			org.ebayopensource.turmeric.runtime.error.model.Error error = errorDAO
					.findErrorByErrorId(id);
			if (error != null) {
		...
		@Override
		public List<MetricGroupData> getMetricsData(
				MetricCriteria metricCriteria,
				MetricResourceCriteria metricResourceCriteria) {
			...
			String groupBy = metricResourceCriteria
			...
			if (ResourceEntity.SERVICE.value().equals(groupBy)) {
				data1 = metricsDAO.findMetricComponentValuesByService(
						metricName, firstStartTime, firstStartTime + duration,
						serverSide, aggregationPeriod, filters);
				data2 = metricsDAO.findMetricComponentValuesByService(
						metricName, secondStartTime,
						secondStartTime + duration, serverSide,
						aggregationPeriod, filters);
			} else if (ResourceEntity.OPERATION.value().equals(groupBy)) {
				data1 = metricsDAO.findMetricComponentValuesByOperation(
						metricName, firstStartTime, firstStartTime + duration,
						serverSide, aggregationPeriod, filters);
				data2 = metricsDAO.findMetricComponentValuesByOperation(
						metricName, secondStartTime,
						secondStartTime + duration, serverSide,
						aggregationPeriod, filters);
			} else if (ResourceEntity.CONSUMER.value().equals(groupBy)) {
				data1 = metricsDAO.findMetricComponentValuesByConsumer(
						metricName, firstStartTime, firstStartTime + duration,
						serverSide, aggregationPeriod, filters);
				data2 = metricsDAO.findMetricComponentValuesByConsumer(
						metricName, secondStartTime,
						secondStartTime + duration, serverSide,
						aggregationPeriod, filters);
			} else if ("Error".equals(groupBy)) {
				metricName = "SoaFwk.Op.Err.%";
				data1 = metricsDAO.findMetricComponentValuesByMetric(
						metricName, firstStartTime, firstStartTime + duration,
						serverSide, aggregationPeriod, filters);
				data2 = metricsDAO.findMetricComponentValuesByMetric(
						metricName, secondStartTime,
						secondStartTime + duration, serverSide,
						aggregationPeriod, filters);
			} else {
		...
		private Double calcDiff(Double count1, Double count2) {
			...
			if (Math.abs(count1) < ZERO_DOUBLE
					&& Math.abs(count2) < ZERO_DOUBLE) {
				diff = ZERO_DOUBLE;
			} else if (Math.abs(count2) < ZERO_DOUBLE
					&& Math.abs(count1) > ZERO_DOUBLE) {
		...
		private CriteriaInfo populateCriteriaInfo(String groupBy,
			...
			criteriaInfo
					.setRoleType(serverSide ? MonitoringSystem.COLLECTION_LOCATION_SERVER
		...
		@Override
		public List<MetricGraphData> getMetricValue(CriteriaInfo criteriaInfo,
				long beginTime, long duration, int aggregationPeriod,
				String autoDelay) {
			...
			boolean serverSide = MonitoringSystem.COLLECTION_LOCATION_SERVER
					.equals(criteriaInfo.getRoleType());

			List<Map<String, Object>> data = metricsDAO
					.findMetricComponentValuesByTimeStamp(metricName,
							beginTime, endTime, serverSide, aggregationPeriod,
							criteriaInfo.getServiceName(),
							criteriaInfo.getOperationName(),
							criteriaInfo.getConsumerName());

			Map<String, Map<String, Object>> map = transformAggregatedMetricComponentValues(
					encodedMetricName, data);
		...
		public List<PolicyMetricData> getPolicyMetricData(long startTime,
				...
				String subjectTypeName, String subjectValue, String effect) {
			return null;
		}
		...
		public List<String> getMetricsMetadata(String resourceEntityType,
			...
			if (resourceEntity != ResourceEntity.SERVICE)
				throw new IllegalArgumentException(
						"Unsupported input ResourceEntity "
								+ resourceEntityType);
			...
			case SERVICE:
				return metricsDAO
						.findMetricServiceAdminNames(resourceEntityName);
			case OPERATION:
		...
		private Map<String, Map<String, Object>> transformAggregatedMetricComponentValues(
				String encodedMetricName, List<Map<String, Object>> rows) {
