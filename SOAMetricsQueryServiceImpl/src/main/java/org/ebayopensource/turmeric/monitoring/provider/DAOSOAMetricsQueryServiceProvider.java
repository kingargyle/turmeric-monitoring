...
public class DAOSOAMetricsQueryServiceProvider implements SOAMetricsQueryServiceProvider
{
     ...
     *
     * @param entityManagerFactory the JPA EntityManagerFactory to inject
     */
    ...
    @Override
    public List<MetricData> getDetailData(String dc, MetricCriteria metricCriteria, MetricResourceCriteria metricResourceCriteria) {
        return delegate.getDetailData(dc, metricCriteria, metricResourceCriteria);
    }

    @Override
    public List<MetricGraphData> getErrorGraph(String serviceName, String operationName, String consumerName, String errorId, String errorCategory, String errorSeverity, MetricCriteria metricCriteria) {
        return delegate.getErrorGraph(serviceName, operationName, consumerName, errorId, errorCategory, errorSeverity, metricCriteria);
    }

    @Override
    public List<ErrorViewData> getErrorMetricsData(String errorType, List<String> serviceName, List<String> operationName, List<String> consumerName, String errorId, String errorCategory, String errorSeverity, String errorName, MetricCriteria metricCriteria) {
        return delegate.getErrorMetricsData(errorType, serviceName, operationName, consumerName, errorId, errorCategory, errorSeverity, errorName, metricCriteria);
    }
    ...
    @Override
    public MetricData getMetricSummaryData(String dc, MetricCriteria metricCriteria, MetricResourceCriteria metricResourceCriteria) {
        return delegate.getMetricSummaryData(dc, metricCriteria, metricResourceCriteria);
    }

    @Override
    public List<MetricGraphData> getMetricValue(CriteriaInfo criteriaInfo, long startTime, long duration, int step, String autoDelay) {
        return delegate.getMetricValue(criteriaInfo, startTime, duration, step, autoDelay);
    }


    @Override
    public List<PolicyMetricData> getPolicyMetricData(long startTime, long endTime, String policyType, String policyName, String serviceName, String operationName, String subjectTypeName, String subjectValue, String effect) {
        return delegate.getPolicyMetricData(startTime, endTime, policyType, policyName, serviceName, operationName, subjectTypeName, subjectValue, effect);
    }

    @Override
    public List<PolicyMetricGraphData> getPolicyMetricDetailData(String policyName, String serviceName, String operationName, String subjectTypeName, String subjectValue, String listType, long startTime, long endTime) {
        return delegate.getPolicyMetricDetailData(policyName, serviceName, operationName, subjectTypeName, subjectValue, listType, startTime, endTime);
    }
    ...
    @Override
    public List<String> getMetricsMetadata(String resourceEntityType, List<String> resourceEntityName, String resourceEntityResponseType) {
    ...
    private static class Target implements SOAMetricsQueryServiceProvider {
        ...
        @Override
        public List<ErrorViewData> getErrorMetricsData(String errorType, List<String> serviceNames, List<String> operationNames, List<String> consumerNames, String errorIdString, String errorCategoryString, String errorSeverityString, String isErrorId, MetricCriteria metricCriteria) {
            ...
            String errorCategory = errorCategoryString == null ? null : ErrorCategory.fromValue(errorCategoryString).value();
            String errorSeverity = errorSeverityString == null ? null : ErrorSeverity.fromValue(errorSeverityString).value();
            ...
            if ("Category".equals(errorType)) {
                errors1 = errorDAO.findErrorValuesByCategory(firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod, errorId, errorCategory, errorSeverity, filters);
                calls1 = metricsDAO.findCallsCount(firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod);
                errors2 = errorDAO.findErrorValuesByCategory(secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod, errorId, errorCategory, errorSeverity, filters);
                calls2 = metricsDAO.findCallsCount(secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod);
            } else if ("Severity".equals(errorType)) {
                errors1 = errorDAO.findErrorValuesBySeverity(firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod, errorId, errorCategory, errorSeverity, filters);
                calls1 = metricsDAO.findCallsCount(firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod);
                errors2 = errorDAO.findErrorValuesBySeverity(secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod, errorId, errorCategory, errorSeverity, filters);
                calls2 = metricsDAO.findCallsCount(secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod);
            } else {
        ...
        public List<MetricGroupData> getMetricsData(MetricCriteria metricCriteria, MetricResourceCriteria metricResourceCriteria) {
            ...
            if (ResourceEntity.SERVICE.value().equals(groupBy)) {
                data1 = metricsDAO.findMetricComponentValuesByService(metricName, firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod, filters);
                data2 = metricsDAO.findMetricComponentValuesByService(metricName, secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod, filters);
            } else if (ResourceEntity.OPERATION.value().equals(groupBy)) {
                data1 = metricsDAO.findMetricComponentValuesByOperation(metricName, firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod, filters);
                data2 = metricsDAO.findMetricComponentValuesByOperation(metricName, secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod, filters);
            } else if (ResourceEntity.CONSUMER.value().equals(groupBy)) {
                data1 = metricsDAO.findMetricComponentValuesByConsumer(metricName, firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod, filters);
                data2 = metricsDAO.findMetricComponentValuesByConsumer(metricName, secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod, filters);
            } else if ("Error".equals(groupBy)) {
                metricName = "SoaFwk.Op.Err.%";
                data1 = metricsDAO.findMetricComponentValuesByMetric(metricName, firstStartTime, firstStartTime + duration, serverSide, aggregationPeriod, filters);
                data2 = metricsDAO.findMetricComponentValuesByMetric(metricName, secondStartTime, secondStartTime + duration, serverSide, aggregationPeriod, filters);
            } else {
            ...
            for (Map.Entry<String, Map<String, Object>> entry : map1.entrySet()) {
                ...
                if (row2 != null) {
                    ...
                    if (count2.intValue() == 0)
                        ...
                        diff = Math.round((count2 - count1) * 100 / count2);
        ...
        private CriteriaInfo populateCriteriaInfo(String groupBy, Map<String, Object> row) {
            ...
            criteriaInfo.setRoleType(serverSide ? MonitoringSystem.COLLECTION_LOCATION_SERVER : MonitoringSystem.COLLECTION_LOCATION_CLIENT);
        ...
        public List<MetricGraphData> getMetricValue(CriteriaInfo criteriaInfo, long beginTime, long duration, int aggregationPeriod, String autoDelay) {
            ...
            List<Map<String, Object>> data = metricsDAO.findMetricComponentValuesByTimeStamp(metricName, beginTime, endTime, serverSide, aggregationPeriod, criteriaInfo.getServiceName(), criteriaInfo.getOperationName(), criteriaInfo.getConsumerName());
        ...
        private Map<String, Map<String, Object>> transformAggregatedMetricComponentValues(String encodedMetricName, List<Map<String, Object>> rows) {
            Map<String, Map<String, Object>> result = new HashMap<String, Map<String, Object>>();
            for (Map<String, Object> row : rows) {
                // Need to create a key that is the same for the N metricComponentDefs
                ...
                if (existingRow != null) {
                    ...
                    if ("CallCount".equals(encodedMetricName)) {
                        if ("count".equals(metricComponentDef.getName())) {
                            // Overwrite the value from the existing row, we are interested in the call count
                            result.put(key, row);
                        }
                    } else if ("ResponseTime".equals(encodedMetricName)) {
                        if ("count".equals(metricComponentDef.getName())) {
