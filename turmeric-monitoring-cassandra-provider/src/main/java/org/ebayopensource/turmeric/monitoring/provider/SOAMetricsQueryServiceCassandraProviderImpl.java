...
public class SOAMetricsQueryServiceCassandraProviderImpl implements
	...
	public SOAMetricsQueryServiceCassandraProviderImpl() {
		...
		metricsErrorByIdDAO = new MetricsErrorByIdDAOImpl<Long>(clusterName,
				host, keyspace, errorByIdCF, Long.class);
		metricsErrorValuesDAO = new MetricsErrorValuesDAOImpl(clusterName,
				host, keyspace, errorValuesCF, String.class);
		metricsErrorsByCategoryDAO = new MetricsErrorsByCategoryDAOImpl<String>(
				clusterName, host, keyspace, errorsByCategoryCF, String.class,
				metricsErrorValuesDAO);
		metricsErrorsBySeverityDAO = new MetricsErrorsBySeverityDAOImpl<String>(
				clusterName, host, keyspace, errorsBySeverityCF, String.class,
				metricsErrorValuesDAO);
		metricIdentifierDAO = new MetricIdentifierDAOImpl<String>(clusterName,
				host, keyspace, metricsCF, String.class);
		metricTimeSeriesDAO = new MetricTimeSeriesDAOImpl<String>(clusterName,
				host, keyspace, metricByTimeSeriesCF, String.class);
		metricsServiceConsumerByIpDAO = new MetricsServiceConsumerByIpDAOImpl<String, String>(
				clusterName, host, keyspace, metricConsumerByIpCF,
				String.class, String.class);
		metricValuesDAO = new MetricValuesDAOImpl<String>(clusterName, host,
				keyspace, metricValuesCF, String.class);
		metricsServiceOperationByIpDAO = new MetricsServiceOperationByIpDAOImpl<String, String>(
				clusterName, host, keyspace, metricServiceOperationByIpCF,
				String.class, String.class);
		metricServiceCallsByTimeDAO = new MetricServiceCallsByTimeDAOImpl<String, Long>(
				clusterName, host, keyspace, metricServiceCallsByTimeCF,
				String.class, Long.class);
		metricValuesByIpAndDateDAO = new MetricValuesByIpAndDateDAOImpl<String, Long>(
				clusterName, host, keyspace, metricValuesByIpAndDateCF,
				String.class, Long.class);
	...
	private void getCassandraConfig() {
		...
		if (inStream != null) {
			...
			try {
				...
				embeed = (String) properties.get(c_hostIp) + ":"
						+ (String) properties.get(c_embeed);
				...
				metricByTimeSeriesCF = (String) properties
						.get(c_metric_timeseries_cf);
				metricConsumerByIpCF = (String) properties
						.get(c_metric_consumer_by_ip_cf);
				metricValuesCF = (String) properties.get(c_metric_values_cf);
				metricServiceOperationByIpCF = (String) properties
						.get(c_metric_service_operation_by_ip_cf);
				metricServiceCallsByTimeCF = (String) properties
						.get(c_metric_service_calls_by_time_cf);
				metricValuesByIpAndDateCF = (String) properties
						.get(c_metric_values_by_ip_and_date_cf);
	...
	public List<MetricGraphData> getErrorGraph(final String serviceName,
		...
		if (errorCategory != null) {
			final String filter = (errorCategory == null ? null : ErrorCategory
					.fromValue(errorCategory).name());
			queryResult = metricsErrorsByCategoryDAO.findErrorValuesByFilter(
					beginTime, endTime, serverSide, aggregationPeriod, null,
					filter, filters);
		} else if (errorSeverity != null) {
			final String filter = (errorSeverity == null ? null : ErrorSeverity
					.fromValue(errorSeverity).name());
			queryResult = metricsErrorsBySeverityDAO.findErrorValuesByFilter(
					beginTime, endTime, serverSide, aggregationPeriod, null,
					filter, filters);
		}
	...
	private double retrieveCallCounts(long startTime, long duration,
			double calls1) {
		List<String> metricValuesList = new ArrayList<String>();
		List<?> valuesByIpAndDateList = metricValuesByIpAndDateDAO.findByRange(
				Long.valueOf(startTime), Long.valueOf(startTime + duration));

		for (Object superModel : valuesByIpAndDateList) {
			Map<Long, Model> superColumns = ((SuperModel<String, Long>) superModel)
					.getColumns();
			...
			for (Entry<Long, Model> superColumn : superColumns.entrySet()) {
				...
				for (Entry<String, Object> column : columns.entrySet()) {
					if (column.getKey().contains(
							SystemMetricDefs.OP_TIME_TOTAL.getMetricName())) {
	...
	public List<String> getMetricsMetadata(String resourceEntityType,
		...
		case SERVICE:
			return metricsServiceOperationByIpDAO
					.findMetricServiceAdminNames(resourceEntityName);
		case OPERATION:
			return metricsServiceOperationByIpDAO
					.findMetricOperationNames(resourceEntityName);
		case CONSUMER: {
			return metricsServiceConsumerByIpDAO
					.findMetricConsumerNames(resourceEntityName);
		}
		default:
			throw new IllegalArgumentException(
